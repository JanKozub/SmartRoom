...
<dom-module id="x-app">
    <template>
        ...
        <iron-ajax auto last-response="{{data}}" url="data/contacts.json"></iron-ajax>
        ...
        <app-drawer-layout fullbleed>
            <div class="content">
                <app-toolbar class="pink-toolbar">
                    ...
                    <div>
                        <paper-icon-button alt="Starred" drawer-toggle icon="star"></paper-icon-button>
                        <paper-badge label$="[[selectedItems.length]]"></paper-badge>
                    </div>
                </app-toolbar>
                <!-- Main List for the items -->
                <iron-list id="itemsList" items="[[data]]" multi-selection selected-items="{{selectedItems}}"
                           selection-enabled>
                    <template>
                        <div>
                            <div aria-label$="Select/Deselect [[item.name]]" class$="[[_computedClass(selected)]]"
                                 tabindex$="[[tabIndex]]">
                                ...
                                <iron-icon class="star" icon$="[[iconForItem(selected)]]"></iron-icon>
                            </div>
            ...
            <app-drawer align="right" slot="drawer">
                <div class="content">
                    ...
                    <template if="[[!selectedItems.length]]" is="dom-if">
                        <div class="no-selection">Select a contact</div>
                    </template>
                    <!-- List for the selected items -->
                    <iron-list hidden$="[[!selectedItems.length]]" id="selectedItemsList" items="[[selectedItems]]">
                        <template>
                            <paper-item aria-label$="Deselect [[item.name]]" on-tap="_unselect"
                                        tabindex$="[[tabIndex]]">[[item.name]]
                            </paper-item>
                        </template>
                    </iron-list>
                </div>
    ...
    <script type="module">
      ...
      Polymer({
        ...
        properties: {selectedItems: {type: Object}},
