...
<head>
    ...
    <meta charset="utf-8">
    <meta content="width=device-width, minimum-scale=1.0, initial-scale=1, user-scalable=no" name="viewport">
...
<dom-module id="x-grid">
    <template>
        ...
        <iron-ajax handle-as="text" id="ajax" loading="{{loadingPhotos}}"
                   on-response="_didReceiveResponse" url="[[_getAPIEndpoint(apiKey, searchText, page)]]"></iron-ajax>

        <app-header effects="waterfall" fixed>
            <app-toolbar>
                <input class="searchInput" placeholder="Search on Flikr" type="search" value="{{searchText::input}}">
            </app-toolbar>
        </app-header>

        <iron-list as="photo" grid items="[[photos]]" scroll-target="document">
            <template>
